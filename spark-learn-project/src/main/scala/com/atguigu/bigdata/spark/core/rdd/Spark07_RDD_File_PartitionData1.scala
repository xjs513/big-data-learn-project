...
object Spark07_RDD_File_PartitionData1 {
  def main(args: Array[String]): Unit = {
    ...
    // *** 区间都是闭区间，之前被读走的忽略不计，同一行向后顺延 ***

    // TODO 1. 分片数 2. 整行读取  3. 基于偏移量

    val lines: RDD[String] = sc.textFile("spark-learn-project\\data\\input\\spark_01\\w.txt", 4)
    println("fileRdd4 partitions:" + lines.partitions.length)
    lines.saveAsTextFile("spark-learn-project\\data\\output\\lines")
